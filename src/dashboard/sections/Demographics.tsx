...
import { ALL_INDIA, CASTE_EDU, CASTE_GROUPS, CASTE_SHARE, EDU_LEVELS, RELIGIONS, STATES } from "../data";
...
export function Demographics() {
  ...
  const filteredCasteEduData = useMemo(() =>
    EDU_LEVELS.map((edu) => {
      const row: Record<string, string | number> = { education: edu };
      const groups = cFilter === "All" ? CASTE_GROUPS : [cFilter];
      for (const c of groups) {
        // Deterministic mock adjustment based on state code
        const stateFactor = state === "IN" ? 1 : (state.charCodeAt(0) + state.charCodeAt(1)) % 10 / 20 + 0.8;
        row[c] = +(CASTE_EDU[c as typeof CASTE_GROUPS[number]][edu] * stateFactor).toFixed(1);
      }
      return row;
    }),
  [cFilter, state]);
  ...
  const stateCasteShare = useMemo(() => {
    ...
    // Mock state-specific caste share
    const entries = Object.entries(CASTE_SHARE).map(([k, v]) => {
      const shift = (state.charCodeAt(0) % 5) - 2; // -2 to +2%
    ...
    <section id="demographics" className="scroll-mt-24 space-y-6">
      ...
      <div className="grid-12">
        ...
        <div className="col-span-12 lg:col-span-7">
          <ChartCard title={`Education attainment · ${s.name}`} subtitle={cFilter === "All" ? "% within each caste" : `Filtered: ${cFilter} only`} delay={50}>
            <div className="h-[280px] md:h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={filteredCasteEduData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                  <XAxis dataKey="education" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} stroke="hsl(var(--border))" />
                  ...
                  {visibleCastes.map((c) => (<Bar key={c} dataKey={c} fill={casteColors[c]} radius={[4, 4, 0, 0]} animationDuration={300} />))}
                </BarChart>
              </ResponsiveContainer>
            </div>
          </ChartCard>
